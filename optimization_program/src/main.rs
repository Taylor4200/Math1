...
fn run_farm(
    ...
    println!("[RUST] Optimization parameters:");
    println!("  num_show_pigs: {}", num_show_pigs);
    println!("  num_pigs_per_fence: {}", num_pigs_per_fence);
    println!("  simulation_trials: {}", simulation_trials);
    println!("  threads_for_fence_construction: {}", threads_for_fence_construction);
    println!("  threads_for_show_construction: {}", threads_for_show_construction);
    println!("  test_spins: {:?}", test_spins);
    println!("  test_spins_weights: {:?}", test_spins_weights);
    println!("  pmb_rtp: {}", pmb_rtp);
    println!("  min_mean_to_median: {}", min_mean_to_median);
    println!("  max_mean_to_median: {}", max_mean_to_median);

    // LOAD IN FORCE OPTIONS AND CONFIG FILE
    println!("[RUST] Loading force options...");
    let force_options = load_force_options(game_name, bet_type, path_to_games.to_string());
    println!("[RUST] Force options loaded: {} entries", force_options.len());
    ...
    println!("Searching for bet_mode_index...");
    while bet_mode_index < config_file.bet_modes.len() && config_file.bet_modes[bet_mode_index].bet_mode != bet_type {
        bet_mode_index += 1;
    }
    if bet_mode_index >= config_file.bet_modes.len() {
        eprintln!("Error: bet_mode '{}' not found in config file", bet_type);
        return;
    }
    println!("Found bet_mode_index: {}", bet_mode_index);
    
    println!("Searching for dress_index...");
    while dress_index < config_file.dresses.len() && config_file.dresses[dress_index].bet_mode != bet_type {
        dress_index += 1;
    }
    if dress_index >= config_file.dresses.len() {
        eprintln!("Error: bet_mode '{}' not found in dresses", bet_type);
        return;
    }
    println!("Found dress_index: {}", dress_index);
    
    println!("Searching for fence_index...");
    while fence_index < config_file.fences.len() && config_file.fences[fence_index].bet_mode != bet_type {
        fence_index += 1;
    }
    if fence_index >= config_file.fences.len() {
        eprintln!("Error: bet_mode '{}' not found in fences", bet_type);
        return;
    }
    println!("Found fence_index: {}", fence_index);
    ...
    {
        Ok(table) => {
            println!("Lookup table loaded: {} entries", table.len());
            io::stdout().flush().unwrap();
            table
        },
        Err(err) => {
    ...
    println!("[RUST] Processing fences...");
    io::stdout().flush().unwrap();
    ...
    println!("[RUST] Bet amount: {}, Target RTP: {}", bet_amount, rtp);
    let mut total_prob = 0.0;
    let mut fences: Vec<Fence> = Vec::new();

    // FIRST THING WE HAVE TO DO IS CREATE EACH FENCE FOR PIGS
    println!("[RUST] Parsing {} fences...", config_file.fences[fence_index].fences.len());
    for fence in &config_file.fences[fence_index].fences {
        println!("  Parsing fence: {}", fence.name);
    ...
    println!("Total probability so far: {}", total_prob);
    ...
    for mut fence in &mut fences {
        println!("\nProcessing fence: {} (hr={}, rtp={}, avg_win={})", fence.name, fence.hr, fence.rtp, fence.avg_win);
        if fence.hr == -1.0 {
            println!("  Calculating hr from remaining probability...");
            fence.hr = 1.0 / (1.0 - total_prob);
            fence.avg_win = fence.hr * fence.rtp;
            println!("  Calculated hr={}, avg_win={}", fence.hr, fence.avg_win);
        }
        println!("\nCreating {} Fence\n", fence.name);
        println!("  Calling sort_wins_by_parameter...");
        sort_wins_by_parameter(&mut fence, &force_options, &mut lookup_table);
        println!("  sort_wins_by_parameter completed. Win dist has {} entries", fence.win_dist.len());
        ...
        if !fence.win_type {
            ...
            // Handle fence with no wins
            if win_array.is_empty() {
                println!("Warning: Fence {} has no wins, using default values", fence.name);
                sorted_wins.push(fence.avg_win);
                // Add a minimal pig pen entry for this fence
                pig_pens.push(vec![Pig {
                    amps: vec![1.0],
                    mus: vec![fence.avg_win * bet_amount],
                    stds: vec![1.0],
                    params: vec![],
                    apply_parms: vec![],
                    rtp: fence.avg_win * bet_amount,
                    sum_dist: 1.0,
                    random_seeds: vec![0],
                    random_weights: vec![0.0],
                    random_apply_params: vec![],
                }]);
            } else {
    ...
    println!("\n[RUST] ========================================");
    println!("[RUST] STARTING SHOW PIG CREATION");
    println!("[RUST] ========================================");
    println!("[RUST] Total show pigs to create: {}", num_show_pigs);
    println!("[RUST] Show pigs per thread: {}", num_show_pigs / threads_for_show_construction);
    println!("[RUST] Number of threads: {}", threads_for_show_construction);
    println!("[RUST] Simulation trials per pig: {}", simulation_trials);
    println!("[RUST] Estimated total simulations: {} ({} show pigs × {} trials)", 
             num_show_pigs * simulation_trials, num_show_pigs, simulation_trials);
    let show_pigs_start = Instant::now();
    ...
    let mut show_pigs: Vec<ShowPig> = thread_pool_show_pigs.install(|| {
        (0..threads_for_show_construction)
            .into_par_iter()
            .enumerate()
            .flat_map(|(thread_id, _)| {
                println!("[RUST] Thread {}: Starting show pig creation...", thread_id);
                let thread_start = Instant::now();
                let result = create_show_pigs(
                    &fences,
                ...
                println!("[RUST] Thread {}: Completed {} show pigs in {:.2?}", 
                        thread_id, result.len(), thread_elapsed);
                result
            })
            .collect()
    });
    
    let show_pigs_elapsed = show_pigs_start.elapsed();
    println!("[RUST] ========================================");
    println!("[RUST] SHOW PIG CREATION COMPLETED");
    println!("[RUST] Total show pigs created: {}", show_pigs.len());
    println!("[RUST] Total time: {:.2?}", show_pigs_elapsed);
    println!("[RUST] Average time per pig: {:.2?}", 
             show_pigs_elapsed / (show_pigs.len() as u32).max(1));
    println!("[RUST] ========================================\n");
        ...
        if pig_index == 0 {
            {
                ...
                // Re-read original lookup table to ensure all entries are preserved
                // The modified lookup_table may have entries removed during fence processing
                let original_lookup: HashMap<u32, LookUpTableEntry> =
                    match read_look_up_table(&game_name, &bet_type, path_to_games.to_string()) {
                        Ok(table) => {
                            println!("[RUST] Re-read original lookup table: {} entries", table.len());
                            table
                        },
                        Err(err) => {
                            eprintln!("Error re-reading lookup table: {}", err);
                            // Fallback to modified table
                            lookup_table.clone()
                        }
                    };
                
                // Merge: use modified weights where available, otherwise use original
                let mut final_lookup = original_lookup.clone();
                for (id, modified_entry) in &lookup_table {
                    if let Some(original_entry) = final_lookup.get_mut(id) {
                        // Update weight from modified entry, keep original win
                        original_entry.weight = modified_entry.weight;
                    } else {
                        // Entry was added (shouldn't happen, but handle it)
                        final_lookup.insert(*id, *modified_entry);
                    }
                }
                
                let mut all_sorted_indexes: Vec<u32> = final_lookup.keys().cloned().collect();
                all_sorted_indexes.sort();
                
                println!("[RUST] Writing lookup table with {} entries (original had {}, modified had {})", 
                         all_sorted_indexes.len(), original_lookup.len(), sorted_indexes.len());

                let mut file = BufWriter::new(File::create(file_path).unwrap());
                for index in &all_sorted_indexes {
                    let entry = final_lookup.get(index).unwrap();
                    // Convert win from multiplier to cents (multiply by 100, round to integer)
                    let win_cents = (entry.win * 100.0).round() as u64;
                    write!(file, "{},{},{}\n", entry.id, entry.weight, win_cents)
...
fn create_show_pigs(
    ...
    println!("[RUST] Thread {}: Creating {} Initial Distributions", process_id, num_pigs);
    println!("[RUST] Thread {}: Parameters - trials={}, bet={}, rtp={}, pmb_rtp={}", 
             process_id, trials, bet_amount, rtp, pmb_rtp);
    ...
    for p in 0..num_pigs {
        ...
        pig_indexes = Vec::new();
        let progress_interval = (num_pigs / 10).max(1); // Log every 10% or at least every pig
        if (p + 1) % progress_interval == 0 || p == 0 {
            let elapsed = create_start.elapsed();
            let percent = 100f32 * ((p + 1) as f32) / (num_pigs as f32);
            let avg_time_per_pig = elapsed / ((p + 1) as u32);
            let remaining_pigs = num_pigs - (p + 1);
            let estimated_remaining = avg_time_per_pig * remaining_pigs;
            println!(
                "[RUST] Thread {}: {}% done ({}/{}) | Elapsed: {:.2?} | Est. remaining: {:.2?} | Best score: {:.6}",
                process_id, percent, p + 1, num_pigs, elapsed, estimated_remaining, best_score
            );
        }
        let mut non_win_type_count: usize = 0;
        for fence in fences {
        ...
        if sim_elapsed.as_secs_f64() > 1.0 {
            println!("[RUST] Thread {}: Pig {} simulation took {:.2?} (trials={})", 
                    process_id, p + 1, sim_elapsed, trials);
        }
        ...
        if score != 0.0 && score >= best_score {
            ...
            if show_pigs.len() % 10 == 0 {
                println!("[RUST] Thread {}: {} valid show pigs found (best score: {:.6})", 
                        process_id, show_pigs.len(), best_score);
            }
        }
    }
    
    let total_elapsed = create_start.elapsed();
    println!("[RUST] Thread {}: Completed all {} pigs in {:.2?} | Valid pigs: {} | Best score: {:.6}", 
             process_id, num_pigs, total_elapsed, show_pigs.len(), best_score);
...
fn sort_wins_by_parameter(
    ...
    println!("    sort_wins_by_parameter: fence.win_type={}, search.len()={}, win_range_start={}, opposite={}", 
             fence.win_type, fence.identity_condition.search.len(), 
             fence.identity_condition.win_range_start, fence.identity_condition.opposite);
    println!("    lookup_table has {} entries, force_options has {} entries", lookup_table.len(), force_options.len());
    if fence.win_type {
        ...
        if i_c.search.is_empty() && i_c.win_range_start == -1.0 && i_c.opposite == false {
            println!("    Adding all {} lookup_table entries to fence...", lookup_table.len());
            let mut count = 0;
            ...
            println!("    Added {} entries to fence win_dist", count);
        } else {
            println!("    Processing {} force_options...", force_options.len());
...
fn create_ancestors(
    ...
    let target_pigs = (pig_heaven.num_pigs as f64).sqrt() as usize;
    println!("[RUST] Creating pig pens: Need {} pos_pigs and {} neg_pigs (target: sqrt({}) = {})", 
             target_pigs, target_pigs, pig_heaven.num_pigs, target_pigs);
    let pig_creation_start = Instant::now();
        ...
        || (neg_pigs.len() as f64) < (pig_heaven.num_pigs as f64).sqrt()
    {
        ...
        // Log progress every 1000 iterations or every 10 seconds
        if loop_count % 1000 == 0 || pig_creation_start.elapsed().as_secs() % 10 == 0 {
            println!("[RUST] Pig creation: loop={}, pos_pigs={}/{}, neg_pigs={}/{}, elapsed={:.2?}", 
                     loop_count, pos_pigs.len(), target_pigs, neg_pigs.len(), target_pigs, 
                     pig_creation_start.elapsed());
        }
        if !go_back_down {
            std_weight = f64::min(
        ...
        let max_iterations = 10 * pig_heaven.num_pigs;  // Increased from 5x to 10x
        if loop_count > max_iterations {
            println!("[RUST] WARNING: Exceeded {} iterations in pig creation loop!", max_iterations);
            println!("[RUST] Current state: pos_pigs={}/{}, neg_pigs={}/{}", 
                     pos_pigs.len(), target_pigs, neg_pigs.len(), target_pigs);
            if neg_pigs.len() > pos_pigs.len() {
                println!("[RUST] Issue: RTP too low - cannot find enough positive pigs");
            } else {
                println!("[RUST] Issue: RTP too high - cannot find enough negative pigs");
            }
            // Break out of loop if we have at least some pigs
            if pos_pigs.len() > 0 && neg_pigs.len() > 0 {
                println!("[RUST] Continuing with available pigs: {} pos, {} neg", 
                         pos_pigs.len(), neg_pigs.len());
                break;
            } else {
                panic!("[RUST] FATAL: Cannot create any valid pigs after {} iterations. Check RTP settings.", max_iterations);
            }
        }
        
        // Log every 5000 iterations
        if loop_count % 5000 == 0 {
            println!("[RUST] Still searching for pigs... loop={}, pos={}/{}, neg={}/{}", 
                     loop_count, pos_pigs.len(), target_pigs, neg_pigs.len(), target_pigs);
        }
    }
...
fn run_simulation(
    ...
    // Calculate prob_less_bet penalty
    let mut count_less_bet = 0.0;
    let total_samples = (trials * spins) as f64;
    for i in 0..(trials * spins) as usize {
        if banks_slice[i] < bet && banks_slice[i] > 0.0 {  // Count non-zero wins less than bet
            count_less_bet += 1.0;
        }
    }
    let prob_less_bet = count_less_bet / total_samples;
    ...
    // Apply prob_less_bet penalty: reduce score if prob_less_bet is high
    // Target prob_less_bet < 0.8, so penalize proportionally
    let prob_less_bet_penalty = if prob_less_bet > 0.8 {
        1.0 - (prob_less_bet - 0.8) * 50.0  // NUCLEAR penalty for prob_less_bet > 0.8
    } else {
        1.0 + (0.8 - prob_less_bet) * 5.0  // MASSIVE bonus for prob_less_bet < 0.8
    };
    
    final_score * prob_less_bet_penalty.max(0.001)  // Allow score to be reduced to 0.1% if necessary
}
