...
class ConstructConditions:
    ...
    def __init__(
        ...
        rtp: float = None,
        av_win: float = None,
        hr: Union[float, str] = None,
        search_conditions=None,
    ):
        # Handle hr="x" as a special case - keep as string for JSON serialization
        hr_is_x = (hr == "x")
        
        # If hr="x", we can't calculate rtp from av_win/hr, so rtp must be provided
        if hr_is_x:
            assert rtp is not None and rtp != "x", "When hr='x', rtp must be provided (cannot be calculated)."
        elif rtp is None or rtp == "x":
            # Calculate rtp from av_win and hr
            assert all(
        ...
        # Count None values, but treat hr="x" as a valid value (not None)
        hr_for_count = None if (hr is None) else hr
        none_count = sum([1 for x in [rtp, av_win, hr_for_count] if x is None])
...
def verify_optimization_input(game_config, opt_dict):
    ...
    for mode_name in optimization_mode_names:
        ...
        assert round(bm_rtp, 5) == round(param_rtp, 5), f"Optimization RTP does not match betmode RTP. BetMode: {bm_rtp}, Optimization: {param_rtp}"
