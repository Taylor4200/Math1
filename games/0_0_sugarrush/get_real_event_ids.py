...
# Game modes for Sugar Rush
modes = [
    {"name": "base", "cost": 1.0},
    {"name": "bonus", "cost": 100.0},
    {"name": "super_bonus", "cost": 500.0},
    {"name": "bonus_booster", "cost": 2.0},
]
...
for mode_info in modes:
    ...
    # AVERAGE PAYOUT: Find events closest to the cost of the mode (bet back for that mode)
    # Target is mode_cost * 100 cents (e.g., if cost is 500x, target is 50000 cents)
    avg_target_cents = int(round(mode_cost * 100))  # Cost in cents = bet back
    
    # Calculate distance from mode cost for each event
    events_with_distance = []
    for e in all_events:
        distance = abs(e['payout_multiplier'] - avg_target_cents)
        events_with_distance.append((e, distance))
    
    # Sort by distance from mode cost
    events_with_distance_sorted = sorted(events_with_distance, key=lambda x: x[1])
    
    # Get the 3 closest to mode cost
    avg_events_data = [e[0] for e in events_with_distance_sorted[:3]]
    
    # MID PAYOUT: Find events closest to 5x the cost
    # Target is 5 * mode_cost * 100 cents (e.g., if cost is 500x, target is 250000 cents = 2500x)
    mid_target_cents = int(round(5 * mode_cost * 100))  # 5x the cost
    
    # Calculate distance from mid target for each event
    events_with_mid_distance = []
    for e in all_events:
        distance = abs(e['payout_multiplier'] - mid_target_cents)
        events_with_mid_distance.append((e, distance))
    
    # Sort by distance from mid target
    events_with_mid_distance_sorted = sorted(events_with_mid_distance, key=lambda x: x[1])
    
    # Get the 3 closest to mid
    mid_events_data = [e[0] for e in events_with_mid_distance_sorted[:3]]
    ...
    print(f"\nAVERAGE PAYOUT EVENTS (target: {mode_cost:.2f}x = bet back = {avg_target_cents} cents):")
    for e in avg_events_data:
        print(f"  System Event ID: {e['id']} (Book ID: {e['book_id']}) -> {e['payout_x']:.2f}x = {e['payout_multiplier']} cents")
    
    print(f"\nMID PAYOUT EVENTS (target: {5 * mode_cost:.2f}x = {mid_target_cents} cents):")
    for e in mid_events_data:
        print(f"  System Event ID: {e['id']} (Book ID: {e['book_id']}) -> {e['payout_x']:.2f}x = {e['payout_multiplier']} cents")
    ...
    results[mode_name] = {
        ...
        "average_payout": {
            ...
            "target_payout_x": mode_cost,  # Bet back (cost of the mode)
        ...
        "mid_payout": {
            "target_payout": mid_target_cents,
            "target_payout_x": 5 * mode_cost,  # 5x the cost
            "event_ids": [e['id'] for e in mid_events_data],
            "actual_payouts": [e['payout_multiplier'] for e in mid_events_data],
            "actual_payouts_x": [e['payout_x'] for e in mid_events_data]
        }
    }
...
for mode_name, data in results.items():
    ...
    print(f"  Mid:  Event IDs {data['mid_payout']['event_ids']} -> {[f'{x:.2f}x' for x in data['mid_payout']['actual_payouts_x']]}")
...
with open(md_file, 'w', encoding='utf-8') as f:
    f.write("# Test Event IDs for Sugar Rush (0_0_sugarrush)\n\n")
    ...
    for mode_name, data in results.items():
        ...
        # Average payout (bet back = cost of mode)
        avg_payouts_str = ", ".join([f"{x:.2f}x" for x in data['average_payout']['actual_payouts_x']])
        f.write(f"### Average Payout Events (target: {data['cost']:.2f}x = bet back)\n")
        ...
        # Mid payout (5x the cost)
        f.write(f"### Mid Payout Events (target: {5 * data['cost']:.2f}x)\n")
        for i, eid in enumerate(data['mid_payout']['event_ids']):
            actual = data['mid_payout']['actual_payouts_x'][i]
            f.write(f"- **Event ID: {eid}** → {actual:.2f}x ({data['mid_payout']['actual_payouts'][i]} cents)\n")
        f.write("\n---\n\n")
    
    # Quick reference table
    f.write("## Quick Reference Summary\n\n")
    f.write("| Mode | Low Event IDs | Average Event IDs | Mid Event IDs | High Event IDs |\n")
    f.write("|------|---------------|-------------------|---------------|----------------|\n")
    for mode_name, data in results.items():
        low_ids = ", ".join([str(x) for x in data['low_payout']['event_ids']])
        avg_ids = ", ".join([str(x) for x in data['average_payout']['event_ids']])
        mid_ids = ", ".join([str(x) for x in data['mid_payout']['event_ids']])
        high_ids = ", ".join([str(x) for x in data['high_payout']['event_ids']])
        f.write(f"| **{mode_name}** | {low_ids} | {avg_ids} | {mid_ids} | {high_ids} |\n")
