...
class ReelGenerator:
    ...
    def __init__(
        ...
        # Default symbols if not provided - Sugar Rush: H1, H2, H3, L1, L2, L3, L4 (NO H4, NO M!)
        if symbols is None:
            self.symbols = ["H1", "H2", "H3", "L1", "L2", "L3", "L4"]  # 7 symbols total
        else:
            self.symbols = symbols
        
        # NO multiplier symbols - multipliers come from cluster hits on grid spots!
        self.multiplier_symbols = []  # Sugar Rush has NO M symbols
        ...
        # Normalize weights to account for scatter only (NO multiplier symbols!)
        base_weight_sum = sum(self.symbol_weights.values())
        self.scatter_weight = scatter_weight
        self.multiplier_weight = 0.0  # NO multiplier symbols - multipliers come from grid spots!
        
        # Adjust base symbol weights to leave room for scatter only
        available_weight = 1.0 - scatter_weight
        for symbol in self.symbol_weights:
            self.symbol_weights[symbol] = (self.symbol_weights[symbol] / base_weight_sum) * available_weight

        # NO multiplier color weighting - Sugar Rush has NO multiplier symbols!
        self.multiplier_color_weights = {}
    ...
    def generate_reel_strip(self, reel_index: int = 0) -> List[str]:
        ...
        # NO multiplier symbols - multipliers come from cluster hits on grid spots in Sugar Rush!
...
def generate_sweet_bonanza_reels(include_wincap: bool = False):
    """
    Generate reel files specifically for Sugar Rush game (6x7 board with multipliers that drop on the board).
    ...
    # Base game reels (BR0.csv) - NO multiplier symbols, lows dominate
    print("Generating base game reels (BR0.csv)...")
    base_generator = ReelGenerator(
        num_reels=7,  # Sugar Rush: 7x7 grid
        num_rows=250,
        scatter_weight=0.03,
        multiplier_weight=0.0,  # NO multiplier symbols - multipliers come from grid spots!
    )
    base_generator.generate_reel_file("reels/BR0.csv", reel_type="basegame")
    
    # Free game reels (FR0.csv) - still low-heavy but with richer scatter mix (NO multiplier symbols)
    ...
    free_generator = ReelGenerator(
        ...
        scatter_weight=0.025,  # Higher scatter for retriggers (3-7 scatters)
        multiplier_weight=0.0,  # NO multiplier symbols - multipliers come from grid spots!
    )
