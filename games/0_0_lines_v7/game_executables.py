...
class GameExecutables(GameCalculations):
    ...
    def get_board_multipliers_sum(self):
        """Get sum of all multiplier symbol values on the board (Green, Blue, Purple, Red only)."""
        total_mult = 0
        multiplier_symbols = self.config.special_symbols.get("multiplier", [])
        
        # Only count multiplier symbols (Green, Blue, Purple, Red)
        for reel_idx, reel in enumerate(self.board):
            for row_idx, symbol in enumerate(reel):
                if symbol.name in multiplier_symbols:
                    if symbol.check_attribute("multiplier"):
                        total_mult += symbol.get_attribute("multiplier")
