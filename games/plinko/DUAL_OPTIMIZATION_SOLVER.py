"""DUAL OPTIMIZATION SOLVER - Solves for BOTH multipliers AND probabilities for natural Plinko feel."""
...
def solve_dual_optimization(mode, target_rtp=0.96):
    """Solve for both multiplier values and probabilities simultaneously."""
    
    print(f"\n{mode.upper()}: Dual optimization for natural Plinko feel at {target_rtp:.1%} RTP")
    ...
    # - Middle multipliers (buckets 1-6, 10-15)
    # - All probabilities
    
    def objective(x):
        """Objective function to minimize."""
        # x contains: [middle_multipliers, probabilities]
        ...
        # Set middle multipliers (left side first, then right side)
        middle_idx = 0
        ...
        # For DEMONIC mode, allow higher prob_less_bet due to 0x buckets
        max_prob_less = 0.85 if mode == "demonic" else 0.80
        prob_less_error = max(0, prob_less - max_prob_less)  # Penalty if > max_prob_less
        
        # Natural progression penalty
        # Encourage smooth multiplier progression like real Plinko games
    ...
    # Initial middle multipliers (decreasing from edge to center)
    if mode == "mild":
        initial_middle_mults = [150, 60, 20, 8, 4, 2, 1, 2, 4, 8, 20, 60, 150]
    elif mode == "sinful":
        initial_middle_mults = [400, 120, 40, 12, 4, 2, 0.5, 2, 4, 12, 40, 120, 400]
    else:  # demonic
        initial_middle_mults = [2500, 600, 150, 40, 8, 2, 2, 8, 40, 150, 600, 2500]
    ...
    for i in range(n_middle):
        if mode == "demonic":
            bounds.append((0.1, 5000))  # More conservative bounds for DEMONIC
        else:
    ...
    # Solve
    print(f"  Solving dual optimization...")
    result = minimize(
        objective,
        x0,
        method='trust-constr',
    ...
    if not result.success:
        print(f"  Trust-constr failed, trying SLSQP...")
        result = minimize(
            objective,
            x0,
            method='SLSQP',
...
# Solve for all modes
print("="*70)
print("DUAL OPTIMIZATION SOLVER - NATURAL PLINKO FEEL")
print("="*70)

results = {}
for mode in ["mild", "sinful", "demonic"]:
    solution = solve_dual_optimization(mode, target_rtp=0.96)
...
print("="*70)
print("SUCCESS! Dual optimization created natural Plinko feel with 96% RTP")
